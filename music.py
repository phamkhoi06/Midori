import math, random, asyncio, functools, itertools, datetime
...
class VoiceState:
    ...
    @loop.setter
    def loop(self, value: bool): self._loop = value

    @property
    def volume(self): return self._volume

    @volume.setter
    def volume(self, value: float): self._volume = value
        ...
        while True:
            ...
            if not self.loop:
                ...
                try:
                    ...
                    await self._ctx.message.send("Bot hasn't used in the voice channel. Bot has left voice channel.")
                    return
    ...
    def play_next_song(self, error=None):
        if error: raise VoiceError(str(error))
...
class Music(commands.Cog):
    ...
    @commands.command(name='summon', aliases=['move', 'm'])
    ...
    async def _summon(self, ctx: commands.Context, *, channel: nextcord.VoiceChannel = None):
        ...
        ctx.voice_state.voice = await destination.connect()
        await ctx.message.add_reaction("👌")
        lprint(ctx, f'Summoned to: {channel}')
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        await ctx.send('Volume of the player set to {}%\nWill take effect on next play.'.format(volume))
        lprint(ctx, f'Volume: {volume}')
    ...
    async def _now(self, ctx: commands.Context):
        ...
        lprint(ctx, 'Checked now playing')
